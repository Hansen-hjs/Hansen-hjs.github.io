...
function confirm(json,taskDetails){
	for(var i = 0; i < json.data.length; i++){
		if(taskDetails==0){
			$("#confirm_content h2").text("已确认事件");
    		$("#confirm_content h2").css({backgroundImage: "url(../img/icon_confirm.png)"});
			...
			$("#confirm_content h2").text("未确认事件");
    		$("#confirm_content h2").css({backgroundImage: "url(../img/icon_unconfirmed.png)"});
	...
	let taskType = $("#task_type").val();                  // 巡检类型
  	let taskDetails = $("#task_details").val();             // 报警类型
...
function list_click(json){
	$(".news li").click(function(){
	    ...
	    $(".p_unconfirmed span").eq(list_index).text("已确认").css({background:"#48acf8"});
		 console.log(newsIndex,bgsize,json.data.length);
	})
}
// 提示框动画出现
function boxShow(){
