...
function home(){
	...
	$.ajax({
	    ...
	    success:function(data){
			 if(data.enterpriseGrade== 0 || 1){
				 $apiNo = data.apiNo;
				 console.log($apiNo,data.enterpriseGrade);
				 ...
				 $("#search_box").bind("keyup",function(){
					 ...
					 if(searchText == ''){ // 如果搜索框输入为空
						 ...
						 add_overlay($teskId,$apiNoclass,$enterpriseName);
						 return false;
					 }
				 ...
				 function add_overlay(){  // 添加标注点
					 for (var i = 0; i < data.enterprise.length; i++) {
						 ...
						 marker[i].addEventListener("click",function(){
							 ...
							 $apiNoclass = data.enterprise[this.index].apiNo;
							 ...
							 sessionStorage.setItem("apiNoclass",$apiNoclass);
							 ...
							 console.log($apiNoclass,$entryId);
				 ...
				 function search_point(sIndex,$apiNoclass,$enterpriseName){	// 搜索的标注物
					 ...
					 marker.addEventListener("click",function(){
						 $apiNoclass = data.enterprise[sIndex].apiNo;
						 ...
						 sessionStorage.setItem("enterpriseName",$enterpriseName);
						 sessionStorage.setItem("teskId",$teskId);
						 // console.log($apiNoclass,$enterpriseName);
						 location.href = "company.html";
					 });
				 }
				 // 启用滚轮放大缩小
			 ...
			 if(data.enterpriseGrade==2){
				 $apiNo = data.apiNo;
				 sessionStorage.setItem("apiNo",$apiNo);
				 sessionStorage.setItem("apiNoclass",$apiNo);
				 location.href="company.html";
			 }
	   }  // 成功方法结束
	}); //  ajax结束
